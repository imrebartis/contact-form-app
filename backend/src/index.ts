...
app.get('*', (req, res, next) => {
  ...
  console.log('Handling SPA route:', req.originalUrl);

  // Check if we have an index.html file to send
  const indexHtmlPath = path.join(frontendDistPath, 'index.html');
  if (fs.existsSync(indexHtmlPath)) {
    // For client-side routes, always return the index.html
    if (req.originalUrl === '/welcome' || req.originalUrl === '/contact-form') {
      console.log('Handling SPA client-side route:', req.originalUrl);
    }
    console.log('Sending index.html for SPA route');
