...
import FormSubmission from '../src/models/form-submission.ts';
...
(runTests ? describe : describe.skip)('Database Operations', () => {
  ...
  it('should delete a contact by id', async () => {
    ...
    const newContact = await formSubmissionService.create(contactData);
    const deletedCount = await formSubmissionService.deleteById(newContact.id);
  ...
  it('should return 0 when deleting non-existent contact', async () => {
    const deletedCount = await formSubmissionService.deleteById(
      'non-existent-id'
    );
